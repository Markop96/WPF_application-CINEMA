...
namespace Bioskop.Forme
{
    public partial class formaPlacanje : Window
    {
        ...
        private void PopuniPadajuceListe()
        {
            ...
            List<string> ListaNacinaPlacanja = new List<string> //kreirana nova lista 
            {
                "Kes", //opcije, mozemo ih dodati koliko god zelimo
                "Kartica",
            };
            cbNacinPlacanja.ItemsSource = ListaNacinaPlacanja; //postavljamo listu kao izvor podataka (combobox obavezno na osnovu imena!)
            ...
            try
            {
                ...
                cbRezervacija.ItemsSource = dtRezervacija.DefaultView;//postavljamo data adapter kao izvor podataka
                cbRezervacija.DisplayMemberPath = "rezervacija"; //Podaci koji ce se pojaviti u okviru padajuce liste!!
                cbRezervacija.SelectedValuePath = "RezervacijaId"; //Podaci koji ce se uzeti kao rezultat --> OSTALE VREDNOSTI SU BITNE SAMO DA SE PODUDARAJU
            }
            catch (SqlException ex)
            {
                MessageBox.Show("Padajuće liste nisu popunjene! " + ex.Message, "Greška", MessageBoxButton.OK, MessageBoxImage.Error);
        ...
        public formaPlacanje(bool azuriraj, DataRowView pomocniRed)
        {
            ...
            if (azuriraj && pomocniRed != null)
            {
                if (pomocniRed.DataView.Table.Columns.Contains("ZaposleniId"))
                    cbZaposleni.SelectedValue = pomocniRed["Zaposleni"];
                if (pomocniRed.DataView.Table.Columns.Contains("RezervacijaId"))
                    cbRezervacija.SelectedValue = pomocniRed["Rezervacija"];
        ...
        private void btnSacuvaj_Click(object sender, RoutedEventArgs e)
        {
            try
            {
                ...
                cmd.Parameters.Add("@ZaposleniId", SqlDbType.Int).Value = cbZaposleni.SelectedValue; //.selectedValue!
